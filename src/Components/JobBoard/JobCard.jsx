...
const JobCard = ({
  ...
  const effectiveSaved =
    typeof isSavedProp === "boolean" ? isSavedProp : isSaved;
  const effectiveApplied =
    typeof isAppliedProp === "boolean" ? isAppliedProp : isApplied;
  const effectiveApplyLoading =
    typeof applyLoadingProp === "boolean" ? applyLoadingProp : applyLoading;
  ...
  const handleSave = async (e) => {
    ...
    if (effectiveApplied)
      return toast.info(
        "You have already applied to this job. Saving is not allowed."
      );
    ...
    try {
      ...
      toast[effectiveSaved ? "info" : "success"](
        effectiveSaved ? "🗑 Job removed from saved" : "✅ Job saved!"
      );
  ...
  const handleViewDetails = (e) => {
    e?.stopPropagation();
    navigate(`/jobs/${normalized.id}`, {
      state: { job: { ...job, ...normalized } },
    });
  };
  ...
  return (
    <div
      ...
      className="relative overflow-hidden rounded-2xl bg-white shadow-md hover:shadow-lg transition-all duration-300 p-4 sm:p-6 mb-5 cursor-pointer border border-[#fcf9f5]"
    >
      {/* Header */}
      <div className="flex justify-between items-start mb-3">
        <div className="flex gap-3 items-center">
          <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl bg-[#fcf9f5] flex items-center justify-center border border-[#fcf9f5]">
            <img
          ...
          <div>
            ...
            <div className="flex items-center gap-1 text-xs sm:text-sm text-gray-500">
              <TrendingUp size={14} className="text-[#fcf9f5]" />
      ...
      {normalized.description && (
        <div
          className="mb-3 p-2 rounded-lg bg-[#fcf9f5] border border-[#fcf9f5] text-xs sm:text-sm text-gray-700 line-clamp-2 prose prose-sm max-w-none"
      ...
      {(normalized.category || normalized.skills.length > 0) && (
        <div className="flex gap-2 mb-3 flex-wrap">
          {normalized.category && (
            <span className="px-2 py-1 text-[11px] sm:text-xs font-semibold bg-[#fcf9f5] rounded-full">
              {normalized.category}
            </span>
          )}
          {normalized.skills.map((skill, idx) => (
            <span
              key={idx}
              className="px-2 py-1 text-[11px] sm:text-xs font-semibold bg-[#fcf9f5] rounded-full"
            >
              {skill}
            </span>
          ))}
      ...
      {/* Footer */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-3 border-t border-[#fcf9f5]">
...
const Detail = ({ icon, text }) => (
  <div className="flex items-center gap-1.5 text-xs sm:text-sm text-gray-600 p-1.5 rounded bg-[#fcf9f5]">
