        ...
        const jobsWithCounts = await Promise.all(
          jobsArray.map(async (job) => {
            try {
              ...
              const appliedUsers = res?.candidatesApplied || [];

              const appliedCount = appliedUsers.length;
              const contactedCount = appliedUsers.filter(
                (c) => c.contacted === true
              ).length;
              ...
              console.error(
                `Error fetching applied count for job ${job._id}`,
                err
              );
              return {
                ...job,
  ...
  return (
    <div className="bg-white p-5 sm:p-6 rounded-2xl shadow-lg border border-gray-200 w-full relative">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6">
        ...
        <button
          onClick={() => navigate("/admin?tab=Job")}
