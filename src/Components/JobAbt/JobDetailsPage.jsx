  ...
  useEffect(() => {
    ...
    const loadNotifications = async () => {
      try {
        ...
        // Find notification related to this job
        const relatedNotifs = (data.notifications || []).filter(
          (n) =>
            n.extraData?.job?._id === job.id || // if job.id is MongoDB id
            n.extraData?.job?.id === job.id     // if job.id is another format
        )

        if (relatedNotifs.length > 0) {
          // Pick latest notification
          const latest = relatedNotifs.sort(
            (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
          )[0]

          setApplicationStatus(latest.title) // e.g., "Application Viewed", "Shortlisted"
        }
      } catch (err) {
...
/>
            ...
            <div className="flex gap-2">
              <button
                onClick={handleApply}
                disabled={loading || applied}
                className={`px-5 py-2.5 rounded-xl font-semibold text-sm shadow-md transition-all ${applied
                  ? "bg-green-500 text-white"
                  : "bg-gradient-to-r from-[#caa057] to-[#caa057] text-white hover:opacity-90"
                  }`}
              >
              ...
              <button
                onClick={handleSave}
                className={`px-4 py-2 rounded-xl font-semibold text-sm border transition shadow-sm ${saved
                  ? "border-[#caa057] text-[#caa057] bg-[#fff1ed]"
                  : "border-gray-300 text-gray-600 hover:bg-gray-50"
                  }`}
              >
      ...
      <div className="lg:hidden fixed bottom-4 left-3 right-3 bg-white/95 backdrop-blur-md border shadow-xl rounded-2xl p-3 flex gap-3 items-center">
        ...
        <button
          onClick={handleApply}
          disabled={loading || applied}
          className={`flex-1 py-2.5 rounded-xl font-semibold text-sm shadow-md transition-all ${applied
            ? "bg-green-500 text-white"
            : "bg-gradient-to-r from-[#caa057] to-[#caa057] text-white hover:opacity-90"
            }`}
        >
        ...
        <button
          onClick={handleSave}
          className={`px-4 py-2 rounded-xl font-semibold text-sm border transition shadow-sm ${saved ? "border-[#caa057] text-[#caa057] bg-[#fff1ed]" : "border-gray-300 text-gray-600 hover:bg-gray-50"
            }`}
        >
          {saved ? "⭐ Saved" : <Bookmark size={18} />}
        </button>
