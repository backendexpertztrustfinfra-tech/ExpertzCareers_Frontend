...
const JobMainCard = ({ job, setApplicationStatus }) => {
  ...
  useEffect(() => {
    ...
    const already = appliedJobs.some((j) => j.jobId === job.id && j.userId === user.uid)
  ...
  return (
    <div className="border rounded-xl p-6 shadow-md space-y-4">
      <div>
        ...
        <p className="text-sm">
          {job.salary || job.SalaryIncentive} • {job.type || job.jobType} in {job.category || job.jobCategory}
        </p>
        {job.openings && (
          <p className="text-sm text-gray-600">
            {job.openings} opening{job.openings !== "1" ? "s" : ""} available
          </p>
        )}
