...
const RecSign = () => {
 ...
 try {
  ...
  if (response?.msg?.toLowerCase().includes("updated")) {
    if (response.token && login) {
      login(response.token, "recruiter", true, formData.name, formData.email);
    }

    await getRecruiterProfile(token); // optional await

    // Ensure navigate runs after state updates
    setTimeout(() => {
