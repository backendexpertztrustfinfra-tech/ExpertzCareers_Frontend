    ...
    return items
      .map((item) => {
        try {
          ...
          let parsedObject = JSON.parse(fixed);
...
const ProfilePage = () => {
  ...
  /**
   * IMPORTANT FIX: This function is changed to save ONLY the essential fields (position/degree, company/instution, duration)
   * to the MongoDB string, mimicking the App developer's desired minimal structure.
   * The UI relies on the robust `parseQualOrExp` function to restore the dates from the 'duration' string when fetching.
   */
      ...
      if (key === "Skill") {
        ...
        const finalString = (arrayData || [])
          .map((q) => {
            ...
            // **FIXED: Only save essential fields + duration for cross-platform compatibility**
            const objectToSave = {
              degree: q.degree,
