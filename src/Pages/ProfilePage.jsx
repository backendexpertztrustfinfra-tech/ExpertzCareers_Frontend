    ...
    return items
      .map((item) => {
        try {
          ...
          parsedObject.pursuing =
            parsedObject.pursuing === true || parsedObject.pursuing === "true";
          parsedObject.currentlyWorking =
            parsedObject.currentlyWorking === true ||
            parsedObject.currentlyWorking === "true";
          if (
            (!parsedObject.startDate || !parsedObject.endDate) &&
            parsedObject.duration
          ) {
            const isPresent = parsedObject.duration.includes("Present");
            const parts = parsedObject.duration
              .split(isPresent ? " - " : / - |-/)
              .map((p) => p.trim());
            if (parts.length >= 2) {
              ...
              if (isPresent) {
                ...
                parsedObject.currentlyWorking =
                  parsedObject.currentlyWorking || false;
      ...
      <div className="flex space-x-2">
        ...
        <Button
          onClick={() => {
            if (!isFormValid()) {
              alert(
                "Please fill in all required fields (Name, instution, Start Date, and End Date/Pursuing)."
              );
              return;
            }
            onSave(form);
...
const EditableQualificationCard = ({
  ...
  const displayDuration = (() => {
    ...
    const end = form.pursuing
      ? "Present"
      : form.endDate
      ? monthYearFormatLocal(form.endDate)
        ...
        {isEditing ? (
          <div className="space-y-4">
            ...
            <input
              ...
              value={form.instution || ""}
              onChange={(e) =>
                setForm((p) => ({ ...p, instution: e.target.value }))
              }
              placeholder="instution / University Name *"
              className="w-full p-2 border rounded"
              required
...
const EditableExperienceCard = ({ experience, index, onSave, onDelete }) => {
  ...
  const displayDuration = (() => {
    // Prefer to use the parsed full dates for display if available
    const start = form.startDate ? monthYearFormatLocal(form.startDate) : "";
    const end = form.currentlyWorking
      ? "Present"
      : form.endDate
      ? monthYearFormatLocal(form.endDate)
      : ""; // Fallback to the duration string from the backend if dates aren't fully populated
...
const ProfilePage = () => {
  ...
  const saveArrayToBackend = async (key, arrayData) => {
    try {
      ...
      if (key === "Skill") {
        ...
        const finalString = (arrayData || [])
          .map((q) => {
            ...
            const end = q?.pursuing
              ? "Present"
              : q.endDate
              ? monthYearFormatLocal(q.endDate)
              : "";
        ...
        const finalString = (arrayData || [])
          .map((e) => {
            ...
            const end = e?.currentlyWorking
              ? "Present"
              : e.endDate
              ? monthYearFormatLocal(e.endDate)
              : "";
            const duration = `${start} - ${end}`;
            const objectToSave = {
              position: e.position,
      ...
      if (!apiRes.ok) {
        throw new Error(
          data.message || `Failed to save data with status: ${apiRes.status}`
        );
      }
  ...
  const handleUpdateProfile = async (payloadOrFormData) => {
    try {
      ...
      if (!apiRes.ok) {
        throw new Error(
          data.message ||
            `Failed to update profile with status: ${apiRes.status}`
        );
      }
      setProfileSafely(data);
              ...
              <div className="relative group text-center md:text-left mb-6 md:mb-0">
                ...
                <div className="relative inline-block">
                  ...
                  <Avatar className="w-32 h-32 sm:w-36 sm:h-36 border-4 border-gradient-to-br from-orange-400 to-yellow-400 shadow-2xl ring-4 ring-orange-200/50">
                                       {" "}
                    <AvatarImage src={profile.profilphoto} alt={profile.name} />
                                       {" "}
                ...
                <div className="mt-4">
                  ...
                  {previewVideo || profile.videoIntro ? (
                    <div className="relative">
                                           {" "}
                      <video
                        src={previewVideo || profile.videoIntro}
                        controls
            ...
            <Card className="bg-gradient-to-br from-[#fff1ed] to-[#fff1ed] border-2 border-[#fff1ed]">
              ...
              <CardContent>
                ...
                <div className="text-center p-8 border-2 border-dashed border-[#caa057] rounded-lg hover:border-[#caa057] transition-colors">
                  ...
                  />
                                   {" "}
                <div className="flex flex-col items-center space-y-4 w-full max-w-md mx-auto p-4 bg-white rounded-lg shadow-md">
  {/* Choose File Button */}
  <Button
    className="flex items-center justify-center bg-gradient-to-r from-[#caa057] to-[#caa057] hover:from-[#b4924c] hover:to-[#b4924c] text-white shadow-lg w-full"
  ...
  {profile.resume && (
    <div className="w-full flex flex-col items-center space-y-2">
      ...
      >
        <Button
          variant="outline"
          className="flex items-center justify-center w-full border-orange-300 text-orange-600 hover:bg-orange-50 bg-transparent"
        >
