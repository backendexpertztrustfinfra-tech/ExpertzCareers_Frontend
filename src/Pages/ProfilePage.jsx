...
//   // If it's a string, handle JSON objects separated by '@'
...
//         } catch {
//           // fallback: register-style single-quoted object
//           const parsed = parseRegisterStyleObject(item);
//           return parsed;
//         }
//       })
...
    resume: u.resume || "",
    // Important: parse according to app's format
...
          onChange={(e) =>
            setForm((p) => ({ ...p, pursuing: e.target.checked }))
          }
        />
        Currently Pursuing
      </label>
...
          onChange={(e) =>
            setForm((p) => ({ ...p, currentlyWorking: e.target.checked }))
          }
        />
...
        <Button
          onClick={() => {
            if (!form.position || !form.companyName) return;
            onSave(form);
          }}
        >
          Save
...
const EditableQualificationCard = ({
  qualification,
  index,
  onSave,
  onDelete,
...
                )}
              </span> */}
              <span>{qualification.duration || "N/A"}</span>
            </div>
...
    if (key === "Skill") {
      const csv = (arrayData || []).map((s) => s.trim()).filter(Boolean).join(", ");
      formData.append("Skill", csv);

    } else if (key === "qualification") {
      // convert each qualification to proper JSON string
      const finalString = (arrayData || [])
        .map((q) => {
          const start = monthYearFormatLocal(q.startDate);
          const end = q?.pursuing ? "Present" : monthYearFormatLocal(q.endDate);
...
          });
        })
        .join("@"); // join objects by "@"
      formData.append("qualification", finalString);

    } else if (key === "Experience") {
      const finalString = (arrayData || [])
        .map((e) => {
          const start = monthYearFormatLocal(e.startDate);
          const end = e?.currentlyWorking ? "Present" : monthYearFormatLocal(e.endDate);
          const duration = `${start}-${end}`;
          return JSON.stringify({
...
    else if (editingField === "designation")
      updatePayload.designation = tempValue;
...
    else if (editingField === "email") updatePayload.useremail = tempValue;
    else if (editingField === "bio") updatePayload.bio = tempValue;
    else if (editingField === "previouscompanyName")
      updatePayload.previouscompanyName = tempValue;
    else if (editingField === "currentSalary")
      updatePayload.previousSalary = tempValue;
    else if (editingField === "expectedSalary")
      updatePayload.salaryExpectation = tempValue;
    else if (editingField === "projects") updatePayload.projectlink = tempValue;
    else if (editingField === "certificationlink")
      updatePayload.certificationlink = tempValue;
    else if (editingField === "portfioliolink")
      updatePayload.portfioliolink = tempValue;

    // Only trigger update if the value has actually changed
    if (profile[editingField] !== tempValue) {
      needsUpdate = true;
      updatePayload[uiToBackendMap[editingField]] = tempValue;
    }
...
    const newSkillsList = profile.Skills.map((s, i) =>
      i === index ? updated : s
    );
    const success = await saveArrayToBackend("Skill", newSkillsList);
...
    const newQualsList = profile.qualification.map((q, i) =>
      i === index ? updated : q
    );
    const success = await saveArrayToBackend("qualification", newQualsList);
...
    const newExpList = profile.experience.map((e, i) =>
      i === index ? updated : e
    );
    const success = await saveArrayToBackend("Experience", newExpList);
...
                      const ok = await saveArrayToBackend(
                        "Skill",
                        newSkillsList
                      );
                      if (ok) {
...
                            <Button
                              size="sm"
                              onClick={() => handleSavePortfolioField(key)}
                            >
                              Save
                            </Button>
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => handleCancelPortfolioEdit(key)}
                            >
                              Cancel
                            </Button>
                          </div>
...
                              setIsEditing(true);
                              if (key === "projects")
                                setTempProjectValue(profile[key] || "");
                              if (key === "certificationlink")
                                setTempCertValue(profile[key] || "");
                              if (key === "portfioliolink")
                                setTempPortfolioValue(profile[key] || "");
                            }}
