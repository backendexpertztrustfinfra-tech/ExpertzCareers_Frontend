...
//   if (!field) return []
//   // backend may return a JSON string for a single object or an array
//   const parsed = safeJSONParse(field, null)
//   if (Array.isArray(parsed)) return parsed
...
const ProfilePage = () => {
 ...
 const saveArrayToBackend = async (key, arrayData) => {
  try {
    ...
    } else if (key === "qualification" || key === "Experience") {
      // Convert each object to JSON string
      const stringified = (arrayData || []).map((item) => JSON.stringify(item))
      // Join all objects with "@" delimiter
      const finalString = stringified.join("@")
      formData.append(key, finalString)
    } else {
