  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 px-4 py-10 max-w-6xl mx-auto">
        {contactCards.map((card, idx) => (
          <a
            key={idx}
            href={card.link}
            className="bg-[#fff1ed] shadow-lg rounded-xl p-6 text-center border-t-4 border-transparent hover:border-[#caa057] transition w-full"
            {...(card.link.startsWith("http") ? { target: "_blank", rel: "noopener noreferrer" } : {})}
          >
            <div className="text-[#caa057] flex justify-center mb-3">
              {card.icon}
            </div>
      ...
      <div className="w-full text-black py-10 px-4 sm:px-6 lg:px-20">
        <div
          className="max-w-6xl mx-auto flex flex-col lg:flex-row justify-between gap-10 bg-[#fff1ed] p-6 sm:p-10 rounded-3xl border-transparent hover:border-[#caa057] transition"
          style={{
            boxShadow:
        ...
        >
          ...
          <form className="flex-1 space-y-6" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              className="bg-[#caa057] text-white font-semibold px-6 py-2 rounded-full hover:bg-[#b4924c] transition w-full sm:w-auto"
            >
              SUBMIT
            </button>
